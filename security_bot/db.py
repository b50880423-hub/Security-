...
class Database:
    ...
    async def ensure_group(self, chat_id, default_mute_minutes=60):
        await self.db.group_settings.update_one(
            {"chat_id": chat_id},
            {"$setOnInsert": {
                "chat_id": chat_id, "lockdown": False, "anti_spam": True,
                "anti_links": True, "explicit_protection": True,
                "raid_protection": True, "default_mute_minutes": default_mute_minutes,
                "updated_at": datetime.now(timezone.utc)
            }},
            upsert=True
        )

    async def settings(self, chat_id, default_mute_minutes=60):
        await self.ensure_group(chat_id, default_mute_minutes)
        return await self.db.group_settings.find_one({"chat_id": chat_id})

    async def set_lockdown(self, chat_id, value, default_mute_minutes=60):
        await self.ensure_group(chat_id, default_mute_minutes)
        await self.db.group_settings.update_one(
            {"chat_id": chat_id},
    ...
    async def incident(self, chat_id, user_id, actor_id, kind, severity, reason, message_id=None, metadata=None):
        result = await self.db.incidents.insert_one({
    ...
    async def action(self, chat_id, user_id, actor_id, action, reason, duration_seconds=None, incident_id=None):
        await self.db.moderation_actions.insert_one({
            ...
            "incident_id": incident_id, "created_at": datetime.now(timezone.utc)
        })
    ...
    async def bot_event(self, chat_id, bot_id, username, added_by, promoted):
        await self.db.bot_registry.update_one(
            ...
            {"$set": {
                "username": username, "added_by": added_by,
                "promoted": promoted, "updated_at": datetime.now(timezone.utc)
